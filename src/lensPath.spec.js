...
test('set', () => {
  expect(set(lensProp('d'), 0, testObj)).toEqual({ a: [{ b:1 }, { b:2 }], d: 0 })
...
test('over', () => {
  expect(over(lensPath('d'), inc, testObj)).toEqual({ a: [{ b:1 }, { b:2 }], d: 4 })
  expect(over(lensPath('a.1.b'), inc, testObj)).toEqual({ a: [{ b:1 }, { b:3 }], d: 3 })
  expect(over(lensProp('X'), identity, testObj)).toEqual({ a: [{ b:1 }, { b:2 }], d: 3, X: undefined })
  expect(over(lensPath('a.0.X'), identity, testObj)).toEqual({ a: [{b: 1, X: undefined}, {b: 2}], d: 3 })
})
