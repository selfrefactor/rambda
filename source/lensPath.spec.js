...
test('set', () => {
	...
	expect(set(lensPath([]), 0, testObj)).toBe(0);
});
...
test('set s (get s) === s', () => {
	expect(set(lensPath(['d']), view(lensPath(['d']), testObj), testObj)).toEqual(
		testObj,
	);
	expect(
		set(lensPath(['a', 0, 'b']), view(lensPath(['a', 0, 'b']), testObj), testObj),
	).toEqual(testObj);
});

test('get (set s v) === v', () => {
	expect(view(lensPath(['d']), set(lensPath(['d']), 0, testObj))).toBe(0);
	expect(
