import { filter, groupBy, indexBy, map, pick, pipe, sortBy } from 'rambda'
...
describe('Remeda vs Rambda: groupBy key inference', () => {
  it('groupBy - Remeda infers literal union keys', () => {
    const items = [
      { category: 'a', val: 1 },
      { category: 'b', val: 2 },
      { category: 'a', val: 3 },
    ...
    // Remeda: infers 'a' | 'b' in key position
    const remedaResult = remedaGroupBy(items, x => x.category)
    expectTypeOf(remedaResult).toEqualTypeOf<
      Partial<Record<'a' | 'b', { category: string; val: number }[]>>
    >()

    // Rambda: groupBy<T, K extends string = string> => always Record<string, ...>
    //   cannot express literal key union
...
describe('Remeda vs Rambda: filter nuances', () => {
  it('filter(Boolean) - Remeda does not exclude true', () => {
    const list = [1, true, false] as const

    // Remeda: filter(data, Boolean) keeps truthy values
    const remedaResult = remedaFilter(list, Boolean)
    // Remeda doesn't have ExcludeFalsy, so true is kept
    expectTypeOf(remedaResult).toEqualTypeOf<(1 | true)[]>()
...
describe('Remeda vs Rambda: indexBy key inference', () => {
  it('indexBy - Remeda infers literal keys', () => {
    ...
    // Remeda: indexBy(data, fn) => Partial<Record<Key, T>>
    const remedaResult = remedaGroupBy(list, x => x.id)
    expectTypeOf(remedaResult).toEqualTypeOf<
      Partial<Record<'a' | 'b', (typeof list)[number]>>
    >()

    // Rambda: indexBy(fn)(list) => { [key: string]: T }
    // Loses literal key info
    const rambdaResult = indexBy(x => x.id)(list)
