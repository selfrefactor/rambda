...
test('should allow additional properties', () => {
  const input = { title: 'You shook me', year: 1969 }
  expect(isValid({ input, schema: { title: 'string' } })).toBeTruthy()
})

test('accepts values as schemas', () => {
  const input = { title: 'You shook me', genre: 'Blues', year: 1969 }
  const schema = { title: 'You shook me', year: 1969 }
  expect(isValid({ input, schema })).toBeTruthy()
})

test('compatible schemas with nested object', () => {
  const input = { foo: 'bar', baz: { a: { b: 'c' } } }
  const invalidInputFirst = { foo: 'bar', baz: { a: { b: 1 } } }
  const invalidInputSecond = { foo: 'bar', baz: { a: { b: [] } } }
  const invalidInputThird = { foo: 'bar', baz: { a: { b: null } } }
  const schema = { foo: 'string', baz: { a: { b: 'string' } } }
...
test('should return true when schema is empty object', () => {
  expect(isValid({ input: { a: 1 }, schema: {} })).toBeTruthy()
})

test('when schema is undefined', () => {
  expect(isValid({ input: { a: 1 }, schema: undefined })).toBeFalsy()
})

test('should return false with invalid schema rule', () => {
  const input = { foo: 'bar', a: {} }
...
test('array of schemas', () => {
  ...
  const basicSchema = { a: String, b: Number }
