...
async function createTestFile(methodName, ramdaSpecs){
  ...
  const content = ramdaSpecs ?
    interpolate(testTemplateWithComment, {
      name : methodName,
      ramdaSpecs,
    }) :
...
async function createTypescriptTestFile(methodName){
  const typescriptTestPath = resolve(__dirname,
    `../../source/${ methodName }-spec.ts`)
...
async function addNewMethod(methodName){
  ...
  const newDescriptions = attachDescription({
